...
void setup() {
  ...
  Serial.print("Initial times to rotate: ");
...
void loop() {
  Serial.println("Enter value for ROTATE, Press ENTER");
  while (Serial.available() == 0) ;  // Wait here until input buffer has a character
  {
      //Side 1
    a = Serial.parseInt();        // new command in 1.0 forward
    Serial.print("Rotating "); Serial.print(a, DEC); Serial.println(" times");
  ...
  while (y < a) // Does 540 rotations, on planetary gear this is a full rotation
  {
  ...
  /*
  At this point y is above ROTATE, so motor will not move anymore until new input is given
   */
  y = 0;
  a = 0;
